...
import { 
  ...
  CheckCircle2, X, Loader2, Copy, Check, ChevronRight 
} from "lucide-react";
  ...
  return (
    <div className="p-4 md:p-8 bg-gray-50 min-h-screen font-sans text-gray-800">
      ...
      <div className="mb-6 md:mb-8">
        ...
        <p className="text-gray-500 text-xs md:text-sm mt-1 uppercase font-bold tracking-tighter">Gerencie seus módulos Datacaixa</p>
      </div>
      ...
      <div className="max-w-5xl mx-auto space-y-4 md:space-y-6">
        ...
        <div className="bg-white p-4 md:p-6 rounded-2xl border border-gray-100 shadow-sm">
          <label className="block text-[10px] font-black text-gray-400 uppercase mb-3 flex items-center gap-2 tracking-widest">
            <CheckCircle2 size={14} className="text-green-500" /> Chave de Ativação
          </label>
          ...
          <div className="p-4 md:p-6">
            ...
            <div className="bg-blue-50 border border-blue-100 p-4 rounded-2xl mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div className="flex items-start gap-3">
                ...
                <p className="text-xs md:text-sm text-blue-900 font-bold">Ative a Renovação Automática e ganhe descontos exclusivos!</p>
              </div>
              <button 
            ...
            <div className="space-y-4">
              ...
              <div className="md:hidden space-y-3">
                {meusProdutos.map((p) => (
                  <div key={p.id} className="p-4 border border-gray-100 rounded-2xl bg-gray-50/50 space-y-4">
                    <div className="flex justify-between items-start">
                      <h3 className="font-black text-gray-800 text-sm tracking-tight">{p.nome}</h3>
                      <span className="text-[10px] font-bold text-gray-400 uppercase">Val: {p.validade}</span>
                    </div>
                    
                    <div className="flex items-center justify-between bg-white p-3 rounded-xl border border-gray-100">
                      <div>
                        <p className="text-[9px] font-black text-gray-400 uppercase">Renovação</p>
                        <select 
                          value={p.meses}
                      ...
                      <div className="text-right">
                        ...
                        <p className="text-sm font-black text-gray-800">R$ {(p.desconto * p.meses).toFixed(2)}</p>
                      </div>
                    </div>
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {meusProdutos.map((p) => (
                      <tr key={p.id} className="group hover:bg-gray-50 transition-colors">
                        <td className="px-4 py-5 font-bold text-gray-800">{p.nome}</td>
                        ...
                        <td className="px-4 py-5 text-right font-black text-gray-900">R$ {(p.desconto * p.meses).toFixed(2)}</td>
                        <td className="px-4 py-5 text-center text-gray-500 font-medium text-xs">{p.validade}</td>
          ...
          <div className="bg-white w-full max-w-sm rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95">
            <div className="p-6">
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-lg font-black text-gray-900 uppercase tracking-tighter">Renovação</h3>
                <button onClick={() => setIsModalOpen(false)} className="p-2 bg-gray-100 rounded-full"><X size={18} /></button>
              </div>
              
              <div className="bg-slate-50 p-5 rounded-2xl mb-8 space-y-2">
                <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest leading-none">Produto Selecionado</p>
                <p className="text-lg font-black text-blue-700 leading-tight">{selectedProduct?.nome}</p>
                <div className="pt-2 flex items-center gap-2 text-xs font-bold text-slate-600 uppercase">
                  <ChevronRight size={14} className="text-blue-500" /> {selectedProduct?.meses} mês(es) de acesso
                </div>
              </div>

...
import { 
  Check, X, Zap, BarChart3, Layers, CreditCard, 
  Loader2, QrCode, Lock, Shield, HelpCircle, Trophy, AlertCircle, Barcode, Calendar, ImageIcon, Maximize2, XCircle, ShoppingCart
} from "lucide-react";
import {
  MdPointOfSale,
} from "react-icons/md";
...
const PRODUCTS = [
  {
    ...
    prices: {
      monthly: null,
      quarterly: { total: 239.00, discountLabel: "10% OFF" },
      semiannual: { total: 449.00, discountLabel: "15% OFF" },
      yearly: { total: 749.00, discountLabel: "30% OFF" },
      biennial: { total: 1249.00, discountLabel: "40% OFF" }
    }
  },
  {
    id: "gestao",
    ...
    prices: {
      monthly: null,
      quarterly: { total: 319.00, discountLabel: "10% OFF" },
      semiannual: { total: 569.00, discountLabel: "20% OFF" },
      yearly: { total: 999.00, discountLabel: "30% OFF" },
      biennial: { total: 1699.00, discountLabel: "40% OFF" }
    }
  },
  {
    id: "combo",
    ...
    highlight: true,
    prices: {
      monthly: { total: 129.00, discountLabel: "" },
      quarterly: { total: 369.00, discountLabel: "Economia" },
      semiannual: { total: 699.00, discountLabel: "Economia" },
      yearly: { total: 1099.00, discountLabel: "Super Oferta" },
      biennial: { total: 1799.00, discountLabel: "Max. Desconto" }
    }
  }
];

// --- COMPONENTE CARROSSEL ATUALIZADO ---
const ProductCarousel = ({ images, onExpand }: { images: string[], onExpand: (src: string) => void }) => {
  ...
  useEffect(() => {
    if (images.length > 1) {
      ...
      }, 4000); // 4 segundos para dar tempo de ver
      return () => clearInterval(timer);
    }
  }, [images.length]);

  return (
    <div className="relative w-full h-56 bg-gray-100 group cursor-zoom-in" onClick={() => onExpand(images[current])}>
      {images.map((src, index) => (
        ...
        >
          ...
          <div className="hidden absolute inset-0 bg-slate-200 flex flex-col items-center justify-center text-slate-400">
             ...
             <span className="text-[10px] font-bold uppercase">Imagem não encontrada</span>
          </div>
        </div>
      ))}
      ...
      {images.length > 1 && (
        <div className="absolute bottom-3 left-0 right-0 flex justify-center gap-1.5 z-10 pointer-events-none">
          {images.map((_, idx) => (
            <div 
              key={idx} 
              className={`w-1.5 h-1.5 rounded-full transition-all shadow-sm ${idx === current ? 'bg-white w-3' : 'bg-white/50'}`}
            />
          ))}
        </div>
      )}
...
// --- COMPONENTE LIGHTBOX (Visualização Expandida) ---
const ImageLightbox = ({ src, onClose }: { src: string, onClose: () => void }) => {
    if (!src) return null;

    return (
        <div className="fixed inset-0 z-[100] bg-black/90 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-300" onClick={onClose}>
            <button 
                onClick={onClose} 
...
export default function LicencasPage() {
  const [selectedCycle, setSelectedCycle] = useState<keyof typeof CYCLES>("yearly");
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="text-center space-y-4 max-w-3xl mx-auto mb-10">
          ...
          <p className="text-slate-500 text-lg">
            Tudo em até 12x SEM JUROS no Cartão de Crédito, ou à vista no Pix ou Boleto Bancário.
          </p>
        </div>
          ...
          {PRODUCTS.map((product) => {
            ...
            return (
              ...
              >
                ...
                <ProductCarousel images={product.images} onExpand={setExpandedImage} />
                ...
                <div className="p-6 pb-0 flex-1">
                  ...
                  <div className="flex items-center gap-3 mb-3">
                    <div className={`p-2 rounded-xl ${product.highlight ? 'bg-blue-100 text-blue-600' : 'bg-slate-100 text-slate-600'}`}>
                  ...
                  <div className="mb-6 pb-6 border-b border-slate-100 min-h-[100px] flex flex-col justify-center">
                    {isAvailable ? (
                      <>
                        <div className="flex items-baseline gap-1">
                            ...
                            <span className="text-4xl font-black text-slate-900">
                              {(priceData.total / CYCLES[selectedCycle].months).toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                            </span>
                            <span className="text-slate-400 font-bold text-xs">/mês</span>
                        </div>
                        <div className="mt-1">
                            <p className="text-xs text-slate-500 font-medium">
                                Total: <strong>R$ {priceData.total.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</strong>
                            </p>
                        </div>
                      </>
                    ) : (
                      <div className="text-center py-3 bg-slate-50 rounded-xl border border-dashed border-slate-200">
                        ...
                        <p className="text-xs font-bold text-slate-600">Plano Indisponível</p>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="space-y-3 mb-6 max-h-[300px] overflow-y-auto custom-scrollbar pr-2">
                    {product.features.map((feature: string, idx: number) => (
                      <div key={idx} className="flex items-start gap-2.5">
                        <div className={`mt-0.5 min-w-[16px] rounded-full p-0.5 ${product.highlight ? 'bg-blue-100 text-blue-600' : 'bg-slate-100 text-slate-500'}`}>
                ...
                <div className="p-6 pt-0 mt-auto">
                  <button
                    onClick={() => isAvailable && handleSelectPlan(product, priceData)}
        ...
        <div className="mt-16 bg-white rounded-3xl p-8 border border-slate-200 shadow-sm flex flex-col md:flex-row items-center justify-between gap-6">
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h3 className="font-bold text-lg text-slate-900">Garantia Incondicional de 7 Dias</h3>
                    <p className="text-slate-500 text-sm">Teste qualquer sistema sem riscos.</p>
                </div>
            </div>
            <div className="flex gap-4 text-sm font-bold text-slate-400">
                <span className="flex items-center gap-2"><Lock size={14}/> Pagamento Seguro</span>
                <span className="flex items-center gap-2"><HelpCircle size={14}/> Suporte Incluso</span>
            </div>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in">
            <div className="bg-white rounded-3xl shadow-2xl p-10 max-w-sm w-full text-center relative overflow-hidden">
                ...
                <p className="text-slate-500 mb-8 leading-relaxed text-sm">
                    A licença <strong>{plan.name}</strong> foi ativada. Um e-mail de confirmação foi enviado.
                </p>
                <button onClick={onClose} className="w-full bg-slate-900 text-white font-bold py-4 rounded-xl hover:bg-slate-800 transition-all cursor-pointer">
                    Acessar Sistema
                </button>
            </div>
        </div>
      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-5xl overflow-hidden flex flex-col md:flex-row max-h-[95vh]">
        ...
        <div className="bg-slate-50 p-8 md:w-5/12 border-r border-slate-200 flex flex-col">
            <div className="flex items-center gap-2 mb-6 text-slate-400">
                <AlertCircle size={16} />
                <span className="text-xs font-black uppercase tracking-widest">Resumo da Compra</span>
            </div>
            ...
            <div className="flex-1 space-y-6">
                ...
                <div className="flex gap-4 p-4 bg-white rounded-2xl border border-slate-200 shadow-sm">
                    ...
                    <div>
                        <h4 className="font-bold text-slate-900 leading-tight">{plan.name}</h4>
                        <p className="text-xs text-slate-500 mt-1">Plano {plan.cycle.label}</p>
                        <div className="mt-2 text-xs text-green-600 font-bold bg-green-50 px-2 py-1 rounded w-fit">
                            {plan.priceData.discountLabel ? `${plan.priceData.discountLabel} Aplicado` : 'Melhor Preço'}
                        </div>
                    </div>
                </div>
                        ...
                        <div className="text-right">
                            ...
                            {paymentMethod === 'credit_card' && installments > 1 && (
                                <span className="text-xs text-slate-400 font-medium">
                                    em {installments}x de R$ {(totalPrice / installments).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                                </span>
                            )}
                        </div>
                    </div>
        ...
        <div className="p-8 md:w-7/12 bg-white overflow-y-auto">
            <div className="flex justify-between items-center mb-6">
                <h2 className="text-xl font-bold text-slate-900">Como você quer pagar?</h2>
                <button onClick={onClose} className="p-2 hover:bg-slate-100 rounded-full text-slate-400 transition-colors cursor-pointer"><X size={20}/></button>
            </div>

            <div className="grid grid-cols-3 gap-3 mb-8">
                <button 
                    onClick={() => setPaymentMethod('credit_card')}
                    className={`py-3 px-2 rounded-xl border-2 flex flex-col items-center justify-center gap-1 transition-all font-bold text-xs cursor-pointer ${paymentMethod === 'credit_card' ? 'border-blue-600 bg-blue-50 text-blue-700' : 'border-slate-100 hover:border-slate-200 text-slate-500'}`}
                >
                    <CreditCard size={20} /> Cartão
                </button>
                <button 
                    onClick={() => setPaymentMethod('pix')}
                    className={`py-3 px-2 rounded-xl border-2 flex flex-col items-center justify-center gap-1 transition-all font-bold text-xs cursor-pointer ${paymentMethod === 'pix' ? 'border-green-500 bg-green-50 text-green-700' : 'border-slate-100 hover:border-slate-200 text-slate-500'}`}
                >
                    <QrCode size={20} /> PIX
                </button>
                <button 
                    onClick={() => setPaymentMethod('boleto')}
                    className={`py-3 px-2 rounded-xl border-2 flex flex-col items-center justify-center gap-1 transition-all font-bold text-xs cursor-pointer ${paymentMethod === 'boleto' ? 'border-orange-500 bg-orange-50 text-orange-700' : 'border-slate-100 hover:border-slate-200 text-slate-500'}`}
                >
                ...
                <div className="space-y-5 animate-in fade-in">
                        ...
                        >
                            {installmentOptions.map(i => (
                                <option key={i} value={i}>
                                    {i}x de R$ {(totalPrice / i).toLocaleString('pt-BR', { minimumFractionDigits: 2 })} sem juros
                                </option>
                            ))}
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <div className="relative">
                            <input type="text" placeholder="0000 0000 0000 0000" className="w-full h-11 pl-10 pr-4 rounded-xl border border-slate-200 outline-none focus:ring-2 focus:ring-blue-500 text-sm font-medium" />
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <div className="relative">
                                <input type="text" placeholder="MM/AA" className="w-full h-11 pl-10 pr-4 rounded-xl border border-slate-200 outline-none focus:ring-2 focus:ring-blue-500 text-sm font-medium" />
                        ...
                        <div>
                            ...
                            <div className="relative">
                                <input type="text" placeholder="123" className="w-full h-11 px-4 rounded-xl border border-slate-200 outline-none focus:ring-2 focus:ring-blue-500 text-sm font-medium" />
                    ...
                    <div>
                        ...
                        <input type="text" placeholder="Como impresso no cartão" className="w-full h-11 px-4 rounded-xl border border-slate-200 outline-none focus:ring-2 focus:ring-blue-500 text-sm font-medium" />
                    </div>
                </div>
            )}
            ...
            {paymentMethod === 'pix' && (
                <div className="flex flex-col items-center justify-center py-6 animate-in fade-in text-center bg-slate-50 rounded-2xl border border-slate-100">
                    ...
                    <p className="text-sm font-bold text-slate-800">Pagamento Instantâneo</p>
                    <p className="text-xs text-slate-500 max-w-xs mt-1">Abra o app do seu banco e escaneie o QR Code. A liberação é imediata.</p>
                </div>
            )}
            ...
            {paymentMethod === 'boleto' && (
                <div className="flex flex-col items-center justify-center py-8 animate-in fade-in text-center bg-slate-50 rounded-2xl border border-slate-100">
                    ...
                    <p className="text-sm font-bold text-slate-800">Pagamento via Boleto</p>
                    <p className="text-xs text-slate-500 max-w-xs mt-1 mb-4">A liberação ocorre em até 1 dia útil após o pagamento.</p>
            ...
            >
                ...
                {isProcessing ? 'Processando...' : `Pagar R$ ${totalPrice.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`}
            </button>

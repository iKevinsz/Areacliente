...
import { salvarConta, excluirConta } from '@/app/actions/financeiro';
...
export default function ContasPagarPage({ contasIniciais = [] }: { contasIniciais?: any[] }) {
  const router = useRouter();
  
  // Função auxiliar para formatar os dados que vêm do banco/props
  const formatarDados = (dados: any[]) => {
    return dados.map(c => ({
      ...c,
      id: c.id,
      valor: Number(c.valor),
      vencimento: new Date(c.vencimento).toISOString().split('T')[0]
    }));
  };

  // Estado inicial
  const [contas, setContas] = useState<Conta[]>(formatarDados(contasIniciais));
  
  // --- 2. NOVO: Sincroniza a tela quando o router.refresh() traz novos dados ---
  useEffect(() => {
    setContas(formatarDados(contasIniciais));
  }, [contasIniciais]);
  ...
  const handleSaveConta = async (e: React.FormEvent) => {
    ...
    const result = await salvarConta({ ...formData, id: editingId });

    if (result.success) {
      setIsModalOpen(false);
      setShowSuccessModal(true);
      router.refresh(); // Isso dispara o useEffect acima e atualiza a lista
    } else {
  ...
  const handleConfirmDelete = async () => {
    ...
    setIsLoading(true);
    const result = await excluirConta(Number(idToDelete));

    if (result.success) {
      setIsDeleteModalOpen(false);
      setIdToDelete(null);
      router.refresh(); // Isso dispara o useEffect acima e remove o item da lista
    } else {
  ...
  const StatusBadge = ({ status }: { status: string }) => {
    ...
    // @ts-ignore
    return <span className={`px-3 py-1 rounded-full text-xs font-semibold border ${styles[status]}`}>{labels[status as keyof typeof labels]}</span>;
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50/50 p-6 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input 
                    ...
                    placeholder="R$ 0,00" 
                    value={formData.valor === 0 ? '' : formData.valor.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })} 
                    onChange={(e) => {
                      const valueOnlyDigits = e.target.value.replace(/\D/g, "");
                      setFormData({...formData, valor: Number(valueOnlyDigits) / 100});
                    }} 
                  />
                </div>
                <div>
      ...
      {/* --- MODAL DE VALIDAÇÃO (NOVO) --- */}
      {isValidationModalOpen && (
        <div className="fixed inset-0 z-[70000] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6 transform transition-all scale-100 flex flex-col items-center text-center">
            ...
            <h3 className="text-xl font-bold text-gray-800 mb-2">Campos Obrigatórios</h3>
            <p className="text-sm text-gray-500 mb-6">
              Por favor, preencha a <strong>Descrição</strong>, o <strong>Valor</strong> e o <strong>Vencimento</strong> para continuar.
            </p>
            <button 
              onClick={() => setIsValidationModalOpen(false)} 
              className="w-full px-4 py-2.5 bg-orange-600 text-white rounded-xl font-bold hover:bg-orange-700 transition-colors shadow-lg shadow-orange-200 cursor-pointer"
            >
              OK, Vou Preencher
            </button>
          </div>
      ...
      {isDeleteModalOpen && (
        <div className="fixed inset-0 z-[60000] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6 transform transition-all scale-100 flex flex-col items-center text-center">
            ...
            <div className="flex gap-3 w-full">
              ...
              <button 
                ...
                className="flex-1 px-4 py-2.5 bg-red-600 text-white rounded-xl font-bold hover:bg-red-700 transition-colors shadow-lg shadow-red-200 cursor-pointer disabled:opacity-70"
                disabled={isLoading}
              >
      ...
      {showSuccessModal && (
        <div className="fixed inset-0 z-[60000] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 relative animate-in zoom-in-95 duration-300 flex flex-col items-center">
            <button onClick={() => setShowSuccessModal(false)} className="absolute top-4 right-4 text-gray-400 hover:text-gray-600 transition-colors cursor-pointer"><X size={20} /></button>
            <div className="w-16 h-16 bg-green-100 rounded-full flex items-center justify-center mb-4"><CheckCircle2 className="text-green-600 w-8 h-8" /></div>
            <h3 className="text-xl font-bold text-gray-900 mb-2">Sucesso!</h3>
            <p className="text-sm text-gray-500 mb-6 text-center">A conta foi salva corretamente no sistema.</p>
            <button onClick={() => setShowSuccessModal(false)} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 rounded-xl transition-all shadow-lg shadow-blue-100 active:scale-95 cursor-pointer">Entendido</button>
          </div>
        </div>
      )}

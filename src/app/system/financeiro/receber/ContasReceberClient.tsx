...
import { 
  ...
  FileText, Trash2, Edit3, TrendingUp, X, Save, Wallet, AlertTriangle, ChevronRight, Loader2
} from 'lucide-react';
import { saveContaReceber, deleteContaReceber, type ContaReceberDTO } from '@/app/actions/contas';

// Reutilizamos a interface da action para garantir compatibilidade
interface ContasReceberClientProps {
  initialContas: ContaReceberDTO[];
}

export default function ContasReceberClient({ initialContas }: ContasReceberClientProps) {
  const router = useRouter();
  
  // Estado sincronizado com o banco
  const [contas, setContas] = useState<ContaReceberDTO[]>(initialContas);

  // Sincroniza quando o router.refresh() traz novos dados
  useEffect(() => {
  ...
  const [formData, setFormData] = useState<ContaReceberDTO>({
    id: null, descricao: '', cliente: '', valor: 0, vencimento: '', categoria: '', status: 'pendente'
  });
  ...
  const handleOpenModal = (conta?: ContaReceberDTO) => {
    if (conta) {
      ...
      setFormData({ id: null, descricao: '', cliente: '', valor: 0, vencimento: hoje, categoria: '', status: 'pendente' });
    }
    setIsModalOpen(true);
  };

  // --- SAVE COM SERVER ACTION ---
  const handleSaveConta = async (e: React.FormEvent) => {
    ...
    try {
        const result = await saveContaReceber(formData);
  ...
  // --- DELETE COM SERVER ACTION ---
  const handleConfirmDelete = async () => {
    if (idToDelete) {
        setIsLoading(true);
        await deleteContaReceber(idToDelete);
        
        // Otimismo: remove da lista visualmente antes do refresh
    ...
    <div className="min-h-screen bg-gray-50/50 p-4 md:p-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-gray-500 text-xs md:text-sm">Gerencie suas receitas futuras.</p>
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-4 justify-between items-start lg:items-center">
          ...
          <div className="flex bg-gray-100 p-1 rounded-lg w-full sm:w-auto overflow-x-auto no-scrollbar">
            {['todos', 'pendente', 'recebido', 'vencido'].map((status) => (
              <button key={status} onClick={() => setFilterStatus(status as any)} className={`flex-1 sm:flex-none px-3 py-1.5 rounded-md text-xs font-medium capitalize transition-all whitespace-nowrap cursor-pointer ${filterStatus === status ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>{status === 'pendente' ? 'a receber' : status}</button>
            ))}
          </div>
        </div>
            ...
            <form onSubmit={handleSaveConta} className="p-6 space-y-4 overflow-y-auto max-h-[80vh]">
              ...
              <div className="pt-4 flex flex-col-reverse sm:flex-row gap-3">
                ...
                <button type="submit" disabled={isLoading} className="flex-1 px-4 py-2.5 bg-blue-600 text-white rounded-xl text-sm hover:bg-blue-700 font-bold flex items-center justify-center gap-2 shadow-lg shadow-blue-100 disabled:opacity-70 active:scale-95 transition-all cursor-pointer">
                  {isLoading ? <Loader2 className="animate-spin" size={16} /> : <><Save size={16} /> Salvar Receita</>}
                </button>
              </div>
              {editingId && (
        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-white w-full max-w-sm rounded-2xl shadow-xl p-6 text-center animate-in zoom-in-95">
            ...
            <div className="flex gap-3">
              ...
              <button onClick={handleConfirmDelete} disabled={isLoading} className="flex-1 px-4 py-2 bg-red-600 text-white rounded-xl font-bold hover:bg-red-700 shadow-lg shadow-red-100 transition-all cursor-pointer">Sim, Excluir</button>
            </div>
          </div>
        </div>
      )}

...
import { responderAvaliacao } from "@/app/actions";
...
export default function AvaliacoesClient({
  ...
  const stats = useMemo(() => {
    ...
    if (total === 0)
      return { media: 0, total: 0, distribuicao: [0, 0, 0, 0, 0] };
    ...
    const distribuicao = counts.map((count) =>
    ...
    <div className="p-4 md:p-6 bg-gray-50 min-h-screen space-y-6 w-full overflow-hidden">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6">
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex flex-col justify-center items-center text-center">
          <h2 className="text-gray-500 font-medium mb-2 text-sm md:text-base">
            Nota Geral
          </h2>
          <div className="flex items-center gap-3">
            ...
            <div className="text-left">
              <div className="flex text-yellow-400 text-base md:text-lg mb-1">
                {[...Array(5)].map((_, i) =>
                  i < Math.round(Number(stats.media)) ? (
                    <FaStar key={i} />
                  ) : (
                    <FaRegStar key={i} className="text-gray-300" />
                  )
        ...
        <div className="lg:col-span-2 bg-white p-6 rounded-xl shadow-sm border border-gray-100">
          <h2 className="text-gray-500 font-medium mb-4 text-sm md:text-base">
          ...
          <div className="space-y-2">
            {[5, 4, 3, 2, 1].map((star, index) => (
              ...
              >
                <span className="w-3 font-bold text-gray-600 shrink-0">
                  {star}
                </span>
                ...
                <span className="w-8 text-right text-gray-400 shrink-0">
                  {stats.distribuicao[index]}%
                </span>
              </div>
            ))}
          </div>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div className="w-full md:w-auto overflow-x-auto pb-1 md:pb-0 scrollbar-hide">
          <div className="flex bg-white p-1 rounded-lg border border-gray-200 shadow-sm min-w-max">
            {/* Adicionado cursor-pointer nos filtros */}
            <button
      ...
      <div className="space-y-4">
        {filteredReviews.map((review) => (
          <div
            key={review.id}
            className="bg-white p-4 md:p-6 rounded-xl shadow-sm border border-gray-100 transition-all hover:border-blue-100 overflow-hidden"
          >
              ...
              <div className="flex items-start gap-4 w-full sm:w-auto min-w-0">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap items-center gap-2 mt-1">
                    ...
                    <span className="text-xs text-gray-400 block sm:inline w-full sm:w-auto mt-1 sm:mt-0">
                      {review.data}
                    </span>
                  </div>
                </div>
            ...
            <div className="pl-0 md:pl-14 flex flex-wrap gap-2 mb-4">
              {review.tags.map((tag) => (
                <span
                  key={tag}
                  className="px-2 py-1 bg-gray-100 text-gray-500 text-xs rounded-md border border-gray-200"
                >
                  {tag}
                </span>
              ))}
                ...
                <>
                  {replyingId === review.id ? (
                    <div className="animate-fade-in bg-gray-50 p-3 md:p-4 rounded-lg border border-gray-200">
                      ...
                      <div className="flex justify-end gap-2 mt-3">
                        <button
                          ...
                          className="px-3 py-2 text-xs md:text-sm text-gray-500 hover:text-gray-700 cursor-pointer disabled:cursor-not-allowed"
                          disabled={isSending}
                        >
                          Cancelar
                        </button>
                        <button
                          ...
                          className="px-4 py-2 bg-blue-600 text-white text-xs md:text-sm rounded-md hover:bg-blue-700 font-medium flex items-center gap-2 disabled:opacity-50 cursor-pointer disabled:cursor-not-allowed"
                        >
                          {isSending ? (
                            "Enviando..."
                          ) : (
                    ...
                    <button
                      ...
                      className="flex items-center gap-2 text-blue-600 text-sm font-medium hover:text-blue-800 hover:bg-blue-50 px-3 py-2 rounded-md transition-colors -ml-3 cursor-pointer"
                    >
                      <FaReply /> Responder
                    </button>
                  )}

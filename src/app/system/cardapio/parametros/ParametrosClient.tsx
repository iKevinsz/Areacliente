...
// --- COMPONENT PRINCIPAL ---
export default function ParametrosClient({ dadosIniciais }: { dadosIniciais: any }) {
  ...
  // --- MODAIS ---
  const openCreateRuleModal = () => { setModalTarget("rule"); setModalType("create"); setEditingItem(tipoTaxa === 'km' ? { min: '', max: '', valor: '' } : { nome: '', valor: '' }); setModalOpen(true); };
  ...
  const handleSaveItem = () => {
    if (modalTarget === 'exception') {
      const newItem = { id: editingItem.id || Date.now(), cep: editingItem.cep, valor: Number(editingItem.valor) };
      if (modalType === 'create') setExcecoesCep([...excecoesCep, newItem]);
      else setExcecoesCep(excecoesCep.map((item:any) => item.id === editingItem.id ? newItem : item));
    } else if (modalTarget === 'pausa') {
      const newItem = { id: editingItem.id || Date.now(), nome: editingItem.nome, inicio: editingItem.inicio, fim: editingItem.fim };
      if (modalType === 'create') setPausas([...pausas, newItem]);
      else setPausas(pausas.map((item:any) => item.id === editingItem.id ? newItem : item));
    } else if (modalTarget === 'cupom') {
      const newItem = { id: editingItem.id || Date.now(), codigo: editingItem.codigo, tipoDesconto: editingItem.tipoDesconto, valor: Number(editingItem.valor), minimoCompra: Number(editingItem.minimoCompra), limiteUso: Number(editingItem.limiteUso), validade: editingItem.validade, ativo: editingItem.ativo, saldoDisponivel: 0 };
      if (modalType === 'create') setCupons([...cupons, newItem]);
      else setCupons(cupons.map((item:any) => item.id === editingItem.id ? newItem : item));
    } else {
      if (tipoTaxa === 'km') {
        const newItem = { id: editingItem.id || Date.now(), min: Number(editingItem.min), max: Number(editingItem.max), valor: Number(editingItem.valor) };
        if (modalType === 'create') setRegrasKm([...regrasKm, newItem]);
        else setRegrasKm(regrasKm.map((item:any) => item.id === editingItem.id ? newItem : item));
      } else if (tipoTaxa === 'bairro') {
        const newItem = { id: editingItem.id || Date.now(), nome: editingItem.nome, valor: Number(editingItem.valor) };
        if (modalType === 'create') setBairros([...bairros, newItem]);
        else setBairros(bairros.map((item:any) => item.id === editingItem.id ? newItem : item));
      }
    }
    closeModal();
    setUnsavedChanges(true); 
  ...
  const handleDeleteItem = () => {
    if (modalTarget === 'exception') setExcecoesCep(excecoesCep.filter((item:any) => item.id !== editingItem.id));
    else if (modalTarget === 'pausa') setPausas(pausas.filter((item:any) => item.id !== editingItem.id));
    else if (modalTarget === 'cupom') setCupons(cupons.filter((item:any) => item.id !== editingItem.id));
    else if (modalTarget === 'schedule') {
      const newHorarios = [...horarios];
      const index = newHorarios.findIndex((h:any) => h.id === editingItem.id);
      if (index >= 0) { newHorarios[index].inicio = ""; newHorarios[index].fim = ""; newHorarios[index].ativo = false; setHorarios(newHorarios); }
    } else {
      if (tipoTaxa === 'km') setRegrasKm(regrasKm.filter((item:any) => item.id !== editingItem.id));
      else if (tipoTaxa === 'bairro') setBairros(bairros.filter((item:any) => item.id !== editingItem.id));
    }
    closeModal();
    setUnsavedChanges(true);
  };
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-6">
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <input type="number" value={valorMinimo} onChange={(e) => {setValorMinimo(e.target.value); setUnsavedChanges(true)}} className="w-full pl-10 pr-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition-all" placeholder="0,00" />
                      </div>
                    </div>
                    <div>
              ...
              <div className="lg:col-span-4 space-y-6">
                ...
                <div className="bg-white p-5 rounded-xl shadow-sm border border-gray-100">
                  ...
                  <div className="mt-6 pt-4 border-t border-gray-100">
                    ...
                    <div className="relative"><span className="absolute left-3 top-2.5 text-gray-400 text-sm font-bold">R$</span><input type="number" value={freteGratis} onChange={(e) => {setFreteGratis(e.target.value); setUnsavedChanges(true)}} className="w-full pl-9 pr-4 py-2 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none" placeholder="0,00" /></div>
                  </div>
                </div>
              </div>
              ...
              <div className="lg:col-span-8 space-y-6">
                <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                  ...
                  <div className="space-y-3">
                    ...
                    <PaymentOption label="Pagamento Online" subLabel="Cartão de Crédito direto no App" icon={<FaGlobe className="text-blue-500"/>} checked={pagamento.onlineCredito} onChange={() => {setPagamento({...pagamento, onlineCredito: !pagamento.onlineCredito}); setUnsavedChanges(true)}}><div className="bg-gray-50 p-3 rounded-lg border border-gray-200"><label className="block text-xs font-bold text-gray-500 uppercase mb-1">Gateway Processador</label><select value={gateways.credito} onChange={(e) => {setGateways({...gateways, credito: e.target.value}); setUnsavedChanges(true)}} className="w-full px-3 py-2 bg-white border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-sm"><option value="asaas">Asaas</option><option value="mercadopago">Mercado Pago</option><option value="stripe">Stripe</option></select></div></PaymentOption>
                    <PaymentOption label="Pix Online (Automático)" subLabel="Confirmação automática via API" icon={<FaQrcode className="text-teal-600"/>} checked={pagamento.onlinePix} onChange={() => {setPagamento({...pagamento, onlinePix: !pagamento.onlinePix}); setUnsavedChanges(true)}}><div className="bg-gray-50 p-3 rounded-lg border border-gray-200"><label className="block text-xs font-bold text-gray-500 uppercase mb-1">Gateway Pix</label><select value={gateways.pix} onChange={(e) => {setGateways({...gateways, pix: e.target.value}); setUnsavedChanges(true)}} className="w-full px-3 py-2 bg-white border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-sm"><option value="asaas">Asaas</option><option value="mercadopago">Mercado Pago</option></select></div></PaymentOption>
                    <PaymentOption label="Pix (Estático)" subLabel="Cliente envia o comprovante" icon={<FaQrcode className="text-gray-600"/>} checked={pagamento.pixEstatico} onChange={() => {setPagamento({...pagamento, pixEstatico: !pagamento.pixEstatico}); setUnsavedChanges(true)}}><div className="space-y-2"><label className="block text-xs font-bold text-gray-500 uppercase">Chave PIX</label><input type="text" value={chavePix} onChange={(e) => {setChavePix(e.target.value); setUnsavedChanges(true)}} className="w-full px-3 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none font-mono text-sm" placeholder="CNPJ, Email ou Telefone" /></div></PaymentOption>
                    <PaymentOption label="Vale Refeição" subLabel="Sodexo, Alelo, VR..." icon={<FaTicketAlt className="text-orange-500"/>} checked={pagamento.valeRefeicao} onChange={() => {setPagamento({...pagamento, valeRefeicao: !pagamento.valeRefeicao}); setUnsavedChanges(true)}}><div className="space-y-2"><label className="block text-xs font-bold text-gray-500 uppercase">Bandeiras Aceitas</label><textarea rows={2} value={bandeirasVale} onChange={(e) => {setBandeirasVale(e.target.value); setUnsavedChanges(true)}} className="w-full px-3 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-sm resize-none" placeholder="Ex: Aceitamos VR e Alelo Refeição..." /></div></PaymentOption>
                  </div>
                </div>
              </div>
              <div className="lg:col-span-4 space-y-6"><div className="bg-blue-50 p-5 rounded-xl border border-blue-100"><div className="flex items-start gap-3"><FaInfoCircle className="text-blue-600 mt-1 shrink-0" /><div><h4 className="font-bold text-blue-900 text-sm mb-1">Como funciona o Pix?</h4><p className="text-xs text-blue-800 leading-relaxed"><strong>Pix Online:</strong> O cliente paga e o sistema aprova o pedido automaticamente na hora. Requer conta em Gateway.<br/><br/><strong>Pix Estático:</strong> O cliente faz a transferência e precisa enviar o comprovante via WhatsApp para você aprovar.</p></div></div></div></div>
            </div>
          )}
      ...
      {showSuccessModal && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in duration-300">
          <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-sm w-full text-center space-y-4 animate-in zoom-in-95">
            ...
            <p className="text-gray-500 text-sm">As configurações foram salvas corretamente!</p>
            <button 
              type="button" 
          ...
          {modalType === 'delete' ? (
            <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6 transform transition-all scale-100">
              <div className="flex flex-col items-center text-center">
                <div className="w-12 h-12 bg-red-100 rounded-full flex items-center justify-center mb-4 text-red-600"><FaExclamationTriangle size={24} /></div>
                <h3 className="text-lg font-bold text-gray-800 mb-2">{modalTarget === 'schedule' ? 'Limpar Horário?' : 'Excluir Item?'}</h3>
                <p className="text-sm text-gray-500 mb-6">{modalTarget === 'schedule' ? 'Deseja remover os horários definidos para este dia? Ele será marcado como fechado.' : 'Tem certeza que deseja remover este item? Esta ação não pode ser desfeita.'}</p>
                <div className="flex gap-3 w-full"><button onClick={closeModal} className="flex-1 px-4 py-2 bg-gray-100 text-gray-700 rounded-lg font-medium hover:bg-gray-200 transition-colors cursor-pointer">Cancelar</button><button onClick={handleDeleteItem} className="flex-1 px-4 py-2 bg-red-600 text-white rounded-lg font-bold hover:bg-red-700 transition-colors shadow-lg shadow-red-200 cursor-pointer">{modalTarget === 'schedule' ? 'Sim, Limpar' : 'Sim, Excluir'}</button></div>
              </div>
            </div>
          ) : (

...
import { 
  ...
  FileText, Trash2, Edit3, TrendingUp, X, Save, Wallet
} from 'lucide-react';
...
const MOCK_CONTAS: ContaReceber[] = [
  { id: '1', descricao: 'Venda de Produtos #1023', cliente: 'Kevin Rodrigo', valor: 350.00, vencimento: '2025-10-25', status: 'pendente', categoria: 'Vendas' },
  { id: '2', descricao: 'Serviço de Consultoria', cliente: 'Empresa XYZ', valor: 1200.00, vencimento: '2025-10-05', status: 'vencido', categoria: 'Serviços' },
  { id: '3', descricao: 'Venda Balcão', cliente: 'Consumidor Final', valor: 85.50, vencimento: '2025-10-10', status: 'recebido', categoria: 'Vendas' },
];
...
export default function ContasReceberPage() {
  ...
  const [formData, setFormData] = useState<Partial<ContaReceber>>({
    descricao: '', cliente: '', valor: 0, vencimento: '', categoria: '', status: 'pendente'
  });
  ...
  const handleOpenModal = (conta?: ContaReceber) => {
    if (conta) {
      ...
      setEditingId(null);
      setFormData({ descricao: '', cliente: '', valor: 0, vencimento: '', categoria: '', status: 'pendente' });
    }
    setIsModalOpen(true);
  };
  ...
  const handleSaveConta = (e: React.FormEvent) => {
    ...
    if (editingId) {
      setContas(prev => prev.map(c => c.id === editingId ? { ...formData, id: editingId } as ContaReceber : c));
    } else {
      const { id, ...dataWithoutId } = formData;
      const novaConta: ContaReceber = {
        id: Math.random().toString(36).substring(2, 11),
        ...dataWithoutId as Omit<ContaReceber, 'id'>
      };
  ...
  return (
    <div className="min-h-screen bg-gray-50/50 p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <KPICard title="A Receber (Previsto)" value={totalPendente} icon={<Wallet className="text-blue-600" />} color="bg-blue-50 border-blue-100" textColor="text-blue-700"/>
        <KPICard title="Vencido (Em Atraso)" value={totalVencido} icon={<AlertCircle className="text-red-600" />} color="bg-red-50 border-red-100" textColor="text-red-700"/>
        <KPICard title="Recebido (Este Mês)" value={totalRecebido} icon={<TrendingUp className="text-green-600" />} color="bg-green-50 border-green-100" textColor="text-green-700"/>
      </div>
      ...
      <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm flex flex-col xl:flex-row gap-4 justify-between items-end xl:items-center">
        <div className="flex flex-col md:flex-row gap-4 w-full xl:w-auto">
          <div className="relative w-full md:w-72">
            ...
            <input type="text" placeholder="Buscar cliente ou descrição..." className="w-full pl-10 pr-4 py-2 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-green-500 outline-none" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
          </div>
                ...
                <tr key={conta.id} className="hover:bg-gray-50/80 transition-colors group cursor-pointer" onClick={() => handleOpenModal(conta)}>
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <div className={`p-2 rounded-lg ${conta.status === 'recebido' ? 'bg-green-50 text-green-600' : 'bg-gray-100 text-gray-600'}`}>
          ...
          <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl overflow-hidden flex flex-col animate-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleSaveConta} className="p-6 space-y-4">
              <div>
                ...
                <input type="text" className="w-full px-3 py-2 border rounded-lg text-sm focus:ring-2 focus:ring-green-500 outline-none" placeholder="Ex: Venda de Mercadoria" 
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="number" step="0.01" className="w-full px-3 py-2 border rounded-lg text-sm focus:ring-2 focus:ring-green-500 outline-none" placeholder="0,00" 
              ...
              <div>
                ...
                <input type="text" className="w-full px-3 py-2 border rounded-lg text-sm focus:ring-2 focus:ring-green-500 outline-none" placeholder="Ex: Nome do Cliente" 
